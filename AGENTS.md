...
- 2 的奖杯卡多了「六块技能牌 + 总星 + 回到关卡列表」，所以**卡片按内容高度、
  游戏区吃掉剩下的高度**（`.finale-top` 是 `flex:0 0 auto`，`.ep` 是 `flex:1 1 auto`），
  不能像 1 那样固定上下各一半，否则卡片会压到计分牌上。
- 2 的通关页可以点「回到关卡列表」离开，离开时 `stopEndless()` 会停掉帧循环、
  清空掉落物并把分数归零，下次进来是新的一局；1 是锁死页面，不需要这一手。
  屏幕高度不足 700px 时，2 会隐藏技能牌，把高度让给游戏区。
