...
学生练到一半手一抖刷新、或者误关标签页，以前会丢掉"这一关做到哪一步"：
闪电反应做到第 4 回合、打地鼠还剩 12 秒——白做一遍。老师要求：
**刷新/退出再进来，进度还在**（只针对这一页）。

成绩（每关最好成绩 + 星级）本来就会存；这次补的是"做到哪一步"。

### 两个存档键

| 键 | 内容 | 谁在写 |
| --- | --- | --- |
| `mouseReactionLab.v1` | 每关的 `best` / `stars` / `plays`（成绩，早就有） | `endGame()` |
| `mouseReactionLab.stage.v1` | `stage[关卡] = {snap, ts}`：这一关做了一半的现场 | 各关卡自己 |
| `mouseReactionLab.finale.v1` | `{ts, score, level}`：在不在通关闭幕里 + 无尽关攒的分和第几关 | `startFinale()` / 无尽关 |

### 规则

- **进关卡就接着做**：`openGame(id)` 发现这一关有现场就接上；首页那一行也会标出
  「· ⏱ 上次做到一半」。想从头做点标题栏的**「重新开始」**（`clearStage()` 后
  `openGame(id,false)`），这是唯一的"从头来"出口，别再加自动清空。
- **各关存什么**：回合制的存已经做完的回合（闪电反应 `times`、描线 `total`），
  拖拽归位还多存已经用掉的时间（星星按总用时评）；计时关存分数 + **剩下的秒数**，
  时钟从上次停下的地方接着走（`startAt = now - (DUR-left)*1000`），
  所以刷新是"暂停"，不是"续命"。
- **写盘节奏**：游戏里每 100ms 报一次现场，`saveStage()` 把写盘限流到 250ms；
  计分、过关这类离散事件传 `true` 立刻写（`saveStage(id,snap,true)`），
  否则刷新后分数会少 1 分，学生看得出来。`pagehide` / `visibilitychange`
  再补一次，最多丢最后不到一秒。
- **过期就丢**：超过 `STAGE_FRESH`（6 小时）没动过的关卡现场、超过
  `FINALE_FRESH`（30 分钟）的庆典现场算过期。学生下一节课打开是正常的关卡列表，
  不会被永远锁在庆典里——首页那条金色横幅还在，点一下就能回到庆典。
  **正在玩的时候 `ts` 会一直滑**，所以玩一节课也不会被判过期。
- **预览不写档**：`?finale=1` 期间 `previewFinale` 为真，星级只标在内存里，
  关卡现场照常可用，但庆典现场既不读也不写，不污染学生机上的真实进度。
...
```powershell
node tools/qa/qa.mjs http://127.0.0.1:8765/ .qa-out 进度存档   # 练一半退出 → 新页面标出「上次做到一半」→ 接着做 + 重新开始
node tools/qa/qa.mjs http://127.0.0.1:8765/ .qa-out 庆典存档   # 在庆典里攒分 → 新页面直接回到庆典，分数接着算
```

这两组都是"前半"把进度做一半就退出、"后半"开一个新页面（等价于刷新，
...
- **唯一的"从头来"出口**是星星那一行末尾的 `↺ 重新开始这一关`（`againOn` 为真时才出现，
  `setStars()` 负责把它挂回行尾）。点了就 `delete stageMap[mode]` 再 `showMode(mode)`。
  那句"接着上次"的提示要在 1.4 秒后先确认 `stageMap` 没被清掉再说，否则重新开始后
  还会迟到地冒出来一句。
