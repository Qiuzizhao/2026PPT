...
- 2 的奖杯卡多了一排**六块技能牌**，所以**卡片按内容高度、游戏区吃掉剩下的高度**
  （`.finale-top` 是 `flex:0 0 auto`，`.ep` 是 `flex:1 1 auto`），
  不能像 1 那样固定上下各一半，否则卡片会压到计分牌上。
- 和 1 一样**不放「总星」和「回到关卡列表」**，这一页也是锁死的：学生只能刷新页面
  回关卡列表（进度已存在 localStorage，刷新不丢）。`hideFinale()` 里保留
  `stopEndless()` 作兜底——将来真加了出口按钮，点一下就能停帧、清场、分数归零。
