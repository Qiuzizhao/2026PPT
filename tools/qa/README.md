...
反应力实验室只存"每关打完的成绩"、不存中途进度（打完一关留星级 + 另一关做一半退出 →
新页面星级还在、关卡从头开始、没有中途存档；在庆典里攒分 → 新页面直接回到庆典、分数接着算）、
鼠标练习营1 的进度存档（左键/双击做一半退出 → 新页面星星还在、导航标出「做了一半」→
进关卡从 4 / 10 接着做、且页面上没有"重新开始"按钮；五关全通后从金色入口进通关页攒分 →
新页面直接回到通关页）、
